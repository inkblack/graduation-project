...
对于运算符、标识符等内容，单独定义枚举类TOKENS来进行记录，对于变量内容（如数字、字母），定义另一个枚举类CHARCLASS来记录。

每当判断出读取内容为TOKENS或是CHARCLASS需要记录的内容时，使用switch来进行分类记录。
...
两个类中，函数继承自变量类，因为函数名同样可以作为一个变量来看，为此需要单独写一个函数bool isFunction来确定变量究竟是否为函数。

### 定义具体内容

void assignmentRule(string) 定义赋值规则，当左边为变量名，右边的值可以赋予时，将值赋给变量。

void expressionRule();void termRule(); 定义加减乘除的规则，可以在值与值，值与变量，变量与变量之间进行执行，由于两者运算级不同，此处分别进行定义。

void factorRule() 对左右括号和引号的规则进行判断，包括出现的场景和具体的含义

bool booleanExprRule() 定义布尔表达式规则，判断条件的真假，当真时返回true,假时返回false。

string evaluate(std::vector<string>) 对得到的算式进行计算并返回值，主要负责进行四则运算部分。
        
void printRule() 输出打印的规则定义


在这部分开发中，由于条件判断、函数定义都需要确认某一个条件作用的定义域范围，所有定义一个定义域类来确定。确定定义域的方法，在多次尝试之后选择使用对于缩进空格的判断。因为按照标准格式输入的代码中，条件判断和函数的具体内容都有缩进。所以这里选择在lexical中添加一个函数processSpaces()单独作为空格、换行和缩进的判断。在parser中就可以更加方便的确认作用域了。由此可以进行以下定义：

void functionRule() 函数规则定义，确定函数范围和函数执行的内容

void functionCallRule(string) 函数调用定义，可以通过函数名进行函数的调用

void ifstmtRule() 条件判断定义，在有作用域确定范围下，可以准确判断if条件作用范围。
