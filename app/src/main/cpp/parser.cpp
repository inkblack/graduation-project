...
void printRule(){
    ...
    try{
        ...
        outputStream += ("\nSyntax Error in line " + to_string(lineNumber) + ": " + s);
        endProgram = true;
    }
}
...
void functionRule(){
    ...
    try{
        ...
        outputStream += ("\nSyntax Error in line " + to_string(lineNumber) + ": " + err);
        endProgram = true;
    }
}
...
bool booleanExprRule(){
    ...
    if(nextToken == COND_EQUAL || nextToken == COND_NOT_OP){
        ...
        if(nextToken == RIGHT_PAREN || nextToken == COLON){
            ...
            if(cond_operator == COND_EQUAL) return std::atoi(operand1.c_str()) == std::atoi(operand2.c_str());
            else if(cond_operator == COND_NOT_OP) return std::atoi(operand1.c_str()) != std::atoi(operand2.c_str());
        }else{
        ...
        log("Bool expr: operand 2 is '%s'\n", operand2.c_str());
        if(nextToken == RIGHT_PAREN || nextToken == COLON){
            return std::atoi(operand1.c_str()) >= std::atoi(operand2.c_str());
        }else{
            throw string("Enclosing characters for condition missing");
        }
...
string evaluate(std::vector<string> s){
    ...
    while(!evaluateStack.empty()){
        if(op_prec(evaluateStack.top().c_str()) == 0){
            ...
            if(op == "+"){
                result = to_string(std::atoi(operand1.c_str()) + std::atoi(operand2.c_str()));
            }else if(op == "-"){
                result = to_string(std::atoi(operand1.c_str()) - std::atoi(operand2.c_str()));
            }else if(op == "*"){
                result = to_string(std::atoi(operand1.c_str()) * std::atoi(operand2.c_str()));
            }else if(op == "/"){
                result = to_string(std::atoi(operand2.c_str()) / std::atoi(operand1.c_str()));
            }
            results.push(result);
            log(" result is %s\n", result.c_str());
        }
